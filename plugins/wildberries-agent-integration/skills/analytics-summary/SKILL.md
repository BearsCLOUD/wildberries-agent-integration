...
Сначала разреши поставщика через `wb_list_suppliers`, затем используй read-only инструмент
`wb_analytics_summary` с его `supplier_id_wb`. Ограничивай ответ текущим пользователем, явно
называй поставщика и период отчёта.

## Входные данные

- `supplier_id_wb` из `wb_list_suppliers` или однозначное имя подключённого поставщика;
