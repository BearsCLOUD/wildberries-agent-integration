...
def test_only_expired_auth_requests_relinking(monkeypatch, status):
    ...
    server = build_server(Settings(
        environment="test", static_access_token="synthetic-test-access",
        public_url="https://wb.example", auth_issuer="https://auth.example",
    ))
...
def test_relink_challenge_survives_http_protocol(monkeypatch):
    ...
    with TestClient(server.streamable_http_app(), base_url="http://127.0.0.1:8080") as client:
