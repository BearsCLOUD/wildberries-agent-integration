...
- Запись себестоимости переведена на серверно связанный двухшаговый протокол: preview с
  `confirmation_token` и TTL 300 секунд, затем отдельное подтверждение с теми же параметрами.
- Просроченное, отсутствующее или несовпадающее подтверждение не выполняет запись; потерянный или
  неоднозначный результат возвращает `write_status_unknown` без автоматической повторной записи.
- Пакет подачи OpenAI синхронизирован с 17 инструментами, 16 навыками и безопасным reviewer-кейсом.

## 0.1.34

- Дополнена политика конфиденциальности: получатели, фактические сроки хранения и удаление данных по запросу в течение 30 дней.
- Reviewer-кейс записи себестоимости приведён к обязательному двухшаговому подтверждению.
- Добавлены естественные стартовые запросы для аналитики, пополнения и юнит-экономики.

## 0.1.33

- Усилена маршрутизация всех 17 инструментов по естественным пользовательским запросам и добавлены описания каждого аргумента.
- Четыре локальных детерминированных анализа доступны без OAuth; приватные данные Seller по-прежнему защищены.
- Reviewer sandbox теперь успешно выполняет все 17 инструментов в свежих MCP-сессиях, включая конкурентов и связь погоды с продажами.
- Унифицированы относительные периоды, запрещён веб-поиск вместо закрытых данных Seller и уточнён двухшаговый сценарий записи себестоимости.

## 0.1.32

- Запись проверки заменена непрерывным сценарием на chatgpt.com: выбор dev-приложения, запрос, вызов MCP и ответ reviewer sandbox.

## 0.1.31

- Пересобрана запись проверки без наложения карточки MCP на итоговую сводку ChatGPT.

## 0.1.30

- Опубликована короткая запись проверки ChatGPT App с вызовом MCP-инструмента в изолированной reviewer sandbox.
...
- Добавлены инструменты статуса подключения и подключения Telegram; сканирование инструментов доступно без входа.
- Два локальных калькулятора объявлены как `noauth`, остальные инструменты защищены OAuth.
- Запись себестоимости требует явного подтверждения; проверочное демо остаётся полностью синтетическим.
- Опубликованы страницы оператора, конфиденциальности, условий и поддержки.

## 0.1.26

- Codex-пакет подключает существующий публичный HTTPS MCP автоматически, без локального Python-сервера.
- Первое использование проходит через существующий Seller OAuth; токен Wildberries не передаётся плагину.
