...
Релиз ориентирован на чтение и расчёты. Единственная запись — подтверждаемая установка себестоимости;
...
| `wb_upload_cost_price` | Записать себестоимость товара в Seller после подтверждения | Seller bearer |
...
Запись себестоимости требует явного `confirm=true` после подтверждения пользователя и меняет
